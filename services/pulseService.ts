...
export const PulseService = {
    ...
    registerMaster: async (data: { name: string; email: string; password: string; academyName: string }) => {
        ...
        // The trigger 'on_auth_user_created' will handle creating:
        // - The 'academies' record
        // - The 'profiles' record
        //
        // NOTE: Supabase Auth can return a 504 timeout on the first attempt while
        // still successfully creating the user. We handle this with retry + graceful
        // "user already registered" treatment (which means the email was sent).
        ...
        // Attempt 1
        const attempt1 = await supabase.auth.signUp(signUpPayload);
        authError = attempt1.error;

        // If the first attempt timed out (504), retry once after a short pause
        if (authError && (authError.status === 504 || authError.message?.includes('timeout') || authError.message?.includes('timed out'))) {
            await new Promise(resolve => setTimeout(resolve, 2000));
            const attempt2 = await supabase.auth.signUp(signUpPayload);
            authError = attempt2.error;
        }

        // "User already registered" means the email WAS sent on a prior timed-out attempt.
        // Treat it as success — the confirmation email is on its way.
        if (authError) {
            const msg = authError.message?.toLowerCase() ?? '';
            const isAlreadyRegistered = msg.includes('user already registered') || msg.includes('already been registered');
            if (!isAlreadyRegistered) {
                throw authError;
            }
            // else: silently succeed — user exists, email was sent
        }
